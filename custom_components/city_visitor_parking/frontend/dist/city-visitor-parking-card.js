...
(() => {
  ...
  const DEFAULT_STRINGS = {
    ...
    "message.select_permit_before_start": "Select a permit before starting a reservation.",
  ...
  const formatDateTimeLocal = (date) => `${formatDate(date)}T${pad(date.getHours())}:${pad(date.getMinutes())}`;
  ...
  class CityVisitorParkingNewReservationCard extends HTMLElement {
    constructor() {
      ...
      __publicField(this, "_deviceEntryId");
      __publicField(this, "_deviceLoadPromise");
      __publicField(this, "_favorites");
      __publicField(this, "_favoritesError");
      __publicField(this, "_favoritesLoadedFor");
      __publicField(this, "_favoritesLoading");
      __publicField(this, "_favoritesByPlate");
      __publicField(this, "_favoritesByPlateName");
      __publicField(this, "_favoritesByValue");
      __publicField(this, "_translationsPromise");
      __publicField(this, "_pendingRemoveFavoriteId");
      __publicField(this, "_selectedEntryId");
      __publicField(this, "_startButtonSuccess");
      __publicField(this, "_startButtonSuccessTimeout");
      __publicField(this, "_startInFlight");
      __publicField(this, "_favoriteRemoveInFlight");
      __publicField(this, "_payWindowEndEntityId");
      __publicField(this, "_payWindowEndEntityDeviceId");
      __publicField(this, "_status");
      __publicField(this, "_statusType");
      __publicField(this, "_appliedThemeVariables");
      __publicField(this, "_appliedThemeName");
      __publicField(this, "_renderHandle");
      __publicField(this, "_favoriteActionsHandle");
      __publicField(this, "_localizeEscapedFn");
      __publicField(this, "_splitDateTimeSupport");
      __publicField(this, "_lastLocaleRef");
      __publicField(this, "_lastThemeName");
      ...
      this._localizeEscapedFn = (key, ...args) => this._escape(this._localize(key, ...args));
    ...
    async _ensureDeviceId() {
      ...
      this._deviceLoadPromise = this._hass.callWS({ type: "config/device_registry/list" }).then((devices) => {
        const match = devices.find((device) => {
          ...
          const hasEntry = Array.isArray(device.config_entries) ? device.config_entries.includes(entryId) : false;
    ...
    _render() {
      ...
      const selectedFavorite = showFavorites ? this._findFavoriteByValue(priorFavorite) : null;
      const selectedFavoriteMatchesLicense = selectedFavorite ? this._selectedFavoriteMatchesLicense(selectedFavorite, currentLicense) : false;
      const matchingFavorite = showFavorites ? this._findFavorite(currentLicense, currentVisitorName) : null;
      const canManageFavorites = !controlsDisabled && !this._favoritesLoading && Boolean(this._deviceId);
      const showAddFavorite = canManageFavorites && currentLicense && currentVisitorName && !matchingFavorite && !selectedFavoriteMatchesLicense;
      const showRemoveFavorite = canManageFavorites && Boolean(selectedFavorite?.id || selectedFavorite?.license_plate) && selectedFavoriteMatchesLicense;
      ...
      const favoritesOptions = (this._favorites || []).map((favorite) => {
      ...
      const reservationFormSection = showReservationForm ? `
        ...
        "field.permit"
          ...
          ${showFavorites ? `
            <div class="row">
              <ha-select id="favorite" label="${localize("field.favorite")}" ${controlsDisabled || this._favoritesLoading ? "disabled" : ""}>
      ...
      this.shadowRoot.innerHTML = `
        ...
        <ha-card>
          ${showHeader ? `<h1 class="card-header"><div class="name">${icon ? `<ha-icon class="icon" icon="${this._escape(icon)}"></ha-icon>` : ""}${this._escape(title)}</div></h1>` : ""}
      ...
      const permitSelect = this.shadowRoot.querySelector(
        "#permitSelect"
      );
      ...
      const favoriteSelectElement = this.shadowRoot.querySelector(
        "#favorite"
      );
      if (favoriteSelectElement) {
        favoriteSelectElement.disabled = controlsDisabled || this._favoritesLoading;
      }
      const now = /* @__PURE__ */ new Date();
      const startDefault = new Date(now.getTime() + 60 * 1e3);
      const endDefault = new Date(startDefault.getTime() + 60 * 60 * 1e3);
      if (useSplitDateTime) {
        const startDateInput = this.shadowRoot.querySelector("#startDate");
        const startTimeInput = this.shadowRoot.querySelector("#startTime");
        const endDateInput = this.shadowRoot.querySelector("#endDate");
        const endTimeInput = this.shadowRoot.querySelector("#endTime");
        ...
        const startDateTimeInput = this.shadowRoot.querySelector("#startDateTime");
        const endDateTimeInput = this.shadowRoot.querySelector("#endDateTime");
      ...
      if (!controlsDisabled) {
        const favoriteSelect = this.shadowRoot.querySelector(
          "#favorite"
        );
        if (favoriteSelect) {
      ...
      const licenseInput = this.shadowRoot.querySelector("#licensePlate");
      if (licenseInput) {
      ...
      const nameInput = this.shadowRoot.querySelector("#visitorName");
      if (nameInput) {
      ...
      if (showFavorites && !controlsDisabled) {
        const addFavorite = this.shadowRoot.querySelector("#addFavorite");
        if (addFavorite) {
          addFavorite.addEventListener("change", () => {
            this._scheduleFavoriteActionsUpdate();
          });
        }
        const removeFavorite = this.shadowRoot.querySelector("#removeFavorite");
        if (removeFavorite) {
    ...
    _updateFavoriteActions() {
      ...
      const canManage = !controlsDisabled && !this._favoritesLoading && this._deviceId;
      const shouldShowAdd = canManage && license && name && !matchingFavorite && !selectedFavoriteMatchesLicense;
      const shouldShowRemove = canManage && Boolean(selectedFavorite?.id || selectedFavorite?.license_plate) && selectedFavoriteMatchesLicense;
      const addWrap = this.shadowRoot.querySelector(
        "#addFavoriteWrap"
      );
      const addCheckbox = this.shadowRoot.querySelector(
        "#addFavorite"
      );
      const removeWrap = this.shadowRoot.querySelector(
        "#removeFavoriteWrap"
      );
      const removeButton = this.shadowRoot.querySelector(
        "#removeFavorite"
      );
      if (addWrap) {
      ...
      if (removeButton) {
        const favoriteId = selectedFavorite?.id || selectedFavorite?.license_plate || "";
    ...
    _resolveTimes() {
      ...
      let end = new Date(start.getTime() + 60 * 60 * 1e3);
      if (useSplitDateTime) {
