...
var getCardConfigForm = async (hassOrLocalize) => {
  ...
  return {
    ...
    computeLabel: (schema) => localize(hassOrLocalize, getFieldKey("editor.field", schema.name)),
...
(() => {
  ...
  class CityVisitorParkingNewReservationCard extends i4 {
    ...
    _getFavoriteActionState() {
      ...
      const showAddFavorite = canManageFavorites && Boolean(license) && Boolean(name) && !matchingFavorite && !selectedFavoriteMatchesLicense;
      ...
      return b2`
        ...
        "action.remove_favorite"
                            ...
                            ` : showAddFavorite ? b2`
                                <ha-formfield
                                  id="addFavoriteWrap"
                                  .label=${localize2("action.add_favorite")}
                                >
                                  <ha-checkbox
                                    id="addFavorite"
                    ...
                    <ha-button
                      id="startReservation"
                      class=${this._startButtonSuccess ? "start-button success" : "start-button"}
                      ?disabled=${startDisabled}
                    >
                      ${localize2("action.start_reservation")}
                    </ha-button>
                  </div>
                ` : A}
            ${this._status ? b2`
