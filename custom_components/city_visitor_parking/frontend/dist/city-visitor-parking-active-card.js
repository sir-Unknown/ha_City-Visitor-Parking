...
var ACTIVE_CARD_CONFIG_FORM = {
  schema: [
    {
      name: "config_entry_id",
      selector: { config_entry: { integration: "city_visitor_parking" } },
      required: false,
      description: "Optional. Limit the list to one permit. Leave empty to show all permits."
    },
    {
      name: "title",
      selector: { text: {} },
...
var getActiveCardConfigForm = () => ACTIVE_CARD_CONFIG_FORM;
...
(() => {
  ...
  class CityVisitorParkingActiveCard extends HTMLElement {
    constructor() {
      ...
      __publicField(this, "_activeReservations");
      __publicField(this, "_activeReservationsError");
      __publicField(this, "_activeReservationsLoadedFor");
      __publicField(this, "_activeReservationsLoading");
      __publicField(this, "_reservationUpdateFieldsByDevice");
      __publicField(this, "_devicesPromise");
      ...
      __publicField(this, "_reservationStartedHandler");
      __publicField(this, "_renderHandle");
      __publicField(this, "_localizeEscapedFn");
      __publicField(this, "_reservationInFlight");
      ...
      this._clickHandler = (event) => this._handleActionClick(event);
    }
    connectedCallback() {
      ...
      try {
        ...
        if (this._config.config_entry_id) {
          devices = devices.filter(
            (device) => (device.config_entries ?? []).includes(this._config.config_entry_id)
          );
        }
        if (!devices.length) {
          this._activeReservations = [];
    ...
    _renderActiveReservations(localize, controlsDisabled) {
      ...
      return `
        ...
        this._activeReservationsError
              ...
              ` : ""}
          ${showEmpty ? `<div class="active-reservations-empty">${localize(
        "message.no_active_reservations"
      )}</div>` : ""}
          ${this._activeReservations.map(
        (reservation) => this._renderActiveReservation(localize, reservation, controlsDisabled)
      ).join("")}
        </div>
      `;
    }
    _renderActiveReservation(localize, reservation, controlsDisabled) {
      ...
      const updateFields = reservation.device_id && this._reservationUpdateFieldsByDevice[reservation.device_id] ? this._reservationUpdateFieldsByDevice[reservation.device_id] : ["start_time", "end_time"];
      ...
      const startDisabledAttr = controlsDisabled || !allowStart || isBusy ? "disabled" : "";
      const endDisabledAttr = controlsDisabled || !allowEnd || isBusy ? "disabled" : "";
      return `
        <div
    ...
    async _handleActiveReservationUpdate(reservationId, trigger) {
      if (!this._hass || !reservationId) {
        return;
      }
      const row = trigger?.closest(".active-reservation");
      if (!row) {
        return;
      }
      ...
      this._requestRender();
      const startInput = row.querySelector(".active-reservation-start");
      const endInput = row.querySelector(".active-reservation-end");
    ...
    async _handleActiveReservationEnd(reservationId, trigger) {
      if (!this._hass || !reservationId) {
        return;
      }
      const row = trigger?.closest(".active-reservation");
    ...
    _formatReservationDateTime(value) {
      ...
      const formatDate = (target) => `${target.getFullYear()}-${pad(target.getMonth() + 1)}-${pad(
        target.getDate()
      )}`;
      const formatDateTimeLocal = (target) => `${formatDate(target)}T${pad(target.getHours())}:${pad(
        target.getMinutes()
      )}`;
